...
void drive::forward(int pwmVa, int pwmVb) {  //Movimiento frontal
  analogWrite(pwmA, pwmVa);                  //Forwards movement
...
void drive::backward(int pwmVa, int pwmVb) {  //Movimiento hacia atra
  analogWrite(pwmA, pwmVa);                    //Backwards movement
  digitalWrite(motA1, LOW);
...
void drive::left(int pwmVa, int pwmVb) {  //Giro hacia la Izquierda, asumiendo que el motor B esta a la derecha
  analogWrite(pwmA, pwmVa);                //Left turn, asuming that motor B is on the left side
  digitalWrite(motA1, HIGH);
  digitalWrite(motA2, LOW);
  analogWrite(pwmB, pwmVb);
  digitalWrite(motB1, LOW);
...
void drive::right(int pwmVa, int pwmVb) {  //Giro hacia la derecha, asumiendo que el motor B esta a la derecha
  analogWrite(pwmA, pwmVa);                 //Right turn, asuming that motor B is on the left side
  digitalWrite(motA1, LOW);
  digitalWrite(motA2, HIGH);
  analogWrite(pwmB, pwmVb);
  digitalWrite(motB1, HIGH);
...
void drive::embrake(int pwmVa, int pwmVb) {  //Freno de emergencia. Usar con precaucion, comprobado solo en el controlador TB6612FNG
  analogWrite(pwmA, pwmVa);                   //Emergency brake. Use with caution, proved only on driver TB6612FNG
  digitalWrite(motA1, HIGH);
  digitalWrite(motA2, HIGH);
  analogWrite(pwmB, pwmVb);
  digitalWrite(motB1, HIGH);
